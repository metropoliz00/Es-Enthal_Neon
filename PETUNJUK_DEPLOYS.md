...
### 2. Set Environment Variables di Netlify / Vercel (WAJIB)
1. Setelah proyek dibuat (atau sebelum deploy), masuk ke **Site configuration / Settings** > **Environment variables**.
2. Tambahkan 2 variabel utama Supabase Anda:
   - **`SUPABASE_URL`**: Project URL Supabase Anda (contoh: `https://xyz.supabase.co` atau connection string `postgresql://...`)
...
2. Klik **Trigger deploy / Redeploy**.
3. Setelah build selesai, buka URL Netlify/Vercel Anda. Aplikasi dan API backend akan berfungsi 100%!

---

## 🔍 Solusi Cepat Jika Masih Belum Connect:
1. **Pastikan SUPABASE_URL & SUPABASE_ANON_KEY Benar**: Salin dari Dashboard Supabase Anda (Project Settings > API).
2. **Jangan gunakan tanda kutip**: Saat memasukkan variabel lingkungan di Vercel atau Netlify, pastikan tidak ada tanda kutip ganda (`"`) atau tunggal (`'`) di awal/akhir string.
...
4. **Jalankan Schema SQL**: Pastikan sudah menempelkan dan menjalankan seluruh skrip SQL dari file `supabase_schema.sql` di SQL Editor Supabase Anda.
