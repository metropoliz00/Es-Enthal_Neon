...
export async function initDbTables() {
  ...
  if (!hasEnv) {
    console.log("DATABASE_URL or SQL_HOST environment variable is not set. Skipping DB table initialization.");
    return false;
  }
  ...
  try {
    const initTask = async () => {
      for (const queryStr of TABLE_SCHEMAS) {
        try {
          await pool.query(queryStr);
        } catch (tableErr: any) {
          console.warn("Table initialization warning:", tableErr?.message || tableErr);
        }
      }
    ...
    const success = await Promise.race([
      ...
      new Promise<boolean>((resolve) => setTimeout(() => {
        console.warn("DB table initialization reached timeout safeguard (6s)");
        resolve(false);
      }, 6000))
    ]);
    ...
    console.error("Failed to auto-initialize DB tables:", err?.message || err);
    return false;
  }
}
