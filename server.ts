...
async function startServer() {
  ...
  app.post("/api/user-config", async (req, res) => {
    const { updates } = req.body; // array of { username, key, value }
    try {
      for (const item of updates) {
        // Since we don't have composite unique constraint in our schema for userConfig,
        // we can check if it exists or insert/update
  ...
  app.post("/api/questions/import", async (req, res) => {
    const { list } = req.body; // Array of { question, optionsList }
    try {
  ...
  app.post("/api/update-user-sessions", async (req, res) => {
    const { updates } = req.body; // array of { username, session }
    try {
  ...
  app.get("/api/recap", async (req, res) => {
    try {
      ...
      // Load all users & exams to perform an in-memory or query join if needed,
      // but simpler: fetch users and exams and combine.
